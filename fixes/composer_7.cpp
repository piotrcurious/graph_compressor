...
#include <complex>
#include <limits>
#include <stdio.h> // For printf (Serial debug output)
...
namespace {
    // Consolidate tolerances per review feedback
    const double kEps = 1e-15;         // General small number for robust comparison
    const double kRootTol = 1e-12;     // Uniqueness for roots
    const double kImagTol = 1e-9;      // Eigenvalue imaginary part tolerance
    const double kLeadCoeffTol = 1e-30;// Min lead coeff for stable root finding
    const double kFuzzyBoundary = 1e-15; // For checking x <= w1
    const double kFeasTol = 1e-8;      // Final QP feasibility tolerance
}
...
std::vector<float> AdvancedPolynomialFitter::composePolynomials(const float* p1_coeffs,
    ...
    auto find_real_roots_unit = [&](const std::vector<double>& poly)->std::vector<double> {
        ...
        // --- SAFER FALLBACK FOR PROBLEM 2 ---
        // Add debug output and skip if lead coeff is too small
        if (std::abs(lead) < kLeadCoeffTol) {
            printf("--- AdvancedPolynomialFitter: Skipping root finding, lead coeff too small (%g). ---\n", lead);
            return roots; // Return empty list (safe fallback)
        }
    ...
    // (Internal logic unchanged per user request, but comparisons made robust)
    ...
    // If we found a solution in the main loop, return it.
    if (std::isfinite(best_err)) {
        return best_coeffs;
    }

    // Main search failed. Start iterative fallback.
    printf("--- AdvancedPolynomialFitter: Main QP search failed. Entering fallback mode. ---\n");
    ...
    // Add ridge
    double trace = ATA0.trace();
    double ridge = 1e-14 * std::max(1.0, std::abs(trace));
    for (int i = 0; i < m; ++i) ATA0(i,i) += ridge;

    // --- Fallback Level 0: C0 Endpoints + C0 Boundary ---
    printf("--- Fallback Level 0: Trying C0 Endpoints + C0 Boundary ---\n");
    ...
    if ((int)C_eq_fb1.size() < m) {
         ...
         if (sol1.first) {
            printf("--- Fallback Level 1 Succeeded. ---\n");
            for (int i = 0; i < m; ++i) best_coeffs[i] = static_cast<float>(sol1.second(i));
            return best_coeffs; // Solution found!
         }
    }
    printf("--- Fallback Level 1 Failed. ---\n");

    // --- Final Fallback: Unconstrained Least Squares ---
    printf("--- All constrained fallbacks failed. Computing unconstrained L2 fit. ---\n");
    ...
    // --- Absolute Failure ---
    printf("--- CRITICAL: All fallbacks failed, even unconstrained solve. Returning empty. ---\n");
    return {};
}
