...
void DataCompressor::logSampledData(const float* data, uint32_t currentTimestamp) {
   ...
   if (dataIndex >= LOG_BUFFER_POINTS_PER_POLY) {
        ...
        delay(5000);// debug - simulate compression delay to produce timestamp skew
        currentPolyIndex++;
        raw_log_delta = 0;       
        dataIndex = 0;
...
#else
//standard        std::vector<float> fitted_sub_abs = fitter.fitPolynomialD_superpos5c(absTimes, values, SUB_FIT_POLY_DEGREE, AdvancedPolynomialFitter::NONE);
        std::vector<float> fitted_sub_abs = fitter.fitPolynomialLebesgueL2_AnalyticProjection(absTimes, values, SUB_FIT_POLY_DEGREE, AdvancedPolynomialFitter::NONE);
